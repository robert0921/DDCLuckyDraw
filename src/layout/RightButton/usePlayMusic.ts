...
export function usePlayMusic() {
    ...
    const { getMusicList: localMusicList, getCurrentMusic: currentMusic, getMusicVolume: musicVolume } = storeToRefs(globalConfig)
    ...
    async function play(item: IMusic) {
        ...
        audio.value.volume = musicVolume.value ?? 1
        audio.value.play()
    }
    ...
    return {
        ...
        playMusic,
        nextPlay,
        setVolume: (v: number) => globalConfig.setMusicVolume(v),
    }
}
